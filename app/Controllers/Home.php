...
public function aksieproduk()
{
    $model = new M_kedaikopi();
    $id_user = session()->get('id'); // Ambil ID user dari session
    $activity = 'Mengubah informasi produk'; // Deskripsi aktivitas
    $this->addLog($id_user, $activity);
...
public function aksibayar() {
    if (session()->get('level') == 2 || session()->get('level') == 0|| session()->get('level') == 1) {
        ...
        foreach ($pesanan as $item) {
            ...
            $data = [
                'kode_pesanan' => $kode_pesanan,
                'tgl_pesanan' => date('Y-m-d H:i:s'), // Menggunakan waktu saat ini sebagai tgl_pesanan
                'id_user' => $id_user,
...
public function laporan()
{
    if (session()->get('id') > 0) {
        ...
        $data['elly'] = session()->get('filter_data') ?? $model->jointigawhere(
            ...
            [
                'pesanan.isdelete' => 0,
                'pesanan.statusbyr' => 'Terbayar'  // Menambah kondisi statusbyr=Terbayar
            ]
        );
...
public function filter()
{
    ...
    if (!empty($tanggal_awal) && !empty($tanggal_akhir)) {
        ...
        $data['elly'] = $model->joinduawhere('pesanan', 'produk', 'pesanan.id_produk = produk.id_produk', 'tgl_pesanan', [
            'pesanan.isdelete' => 0,
            'pesanan.statusbyr' => 'Terbayar'  // Menambah kondisi statusbyr=Terbayar
        ]);
    }
...
public function level()
    {
        if (session()->get('id') > 0) {
            ...
            if (has_permission('level')) {
                $model = new M_kedaikopi();
                $id_user = session()->get('id');
                $activity = 'Mengakses halaman level akses'; // Deskripsi aktivitas
        $this->addLog($id_user, $activity);
        $where=array(
            'id_toko'=> 1
          );
        ...
        $data['elly'] = $model->tampil('user','id_user');
                ...
                helper('permission');
                $data['currentMenu'] = 'level'; // Sesuaikan dengan menu yang aktif
                echo view('header', $data);
                echo view('menu', $data);
